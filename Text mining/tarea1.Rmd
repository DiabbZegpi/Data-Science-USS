...
```{r warning=FALSE}
text <- read.delim("Text Mining/chat_wsp.txt", 
                   encoding = "UTF-8",
                   quote = "",
                   sep = "\n",
                   col.names = "text",
...
word_counts <- chat_clean %>% 
  ...
  filter(!stem %in% stopwords(kind = 'es')) %>% 
  count(msg_id, stem)
...
lda_models <- readRDS(file = here("Text Mining", "lda_models.rds"))
```
...
```{r}
best_model <- lda_models[[3]] # modelo de 4 tópicos
...
En el gráfico siguiente se aprecia que los stem del tópico 1 se relacionan con classroom y proyectos (asignaturas proyecto de grado I, evaluación de proyectos y gestión de proyectos); el tópico 2 trata de saludos matutinos tales como "buenos días a todos"; el tópico 3 puede ser misceláneo, porque lo gobiernan adverbios de cantidad y preposiciones; el tópico 4 trata de tareas, emails y consultas a los/as profesores/as.
...
top_terms %>%
  ...
  labs(title = "Distribución de Probabilidad<br>Top 10 stem por tópico",
