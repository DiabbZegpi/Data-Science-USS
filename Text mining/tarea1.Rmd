...
1. Agrupar a los remitentes menos frecuentes en la categoría "Otro".
2. Tokenizar los mensajes.
3. Excluir las stopwords en español de la lista SnowballC.
4. Lematizar con spaCy.
5. Filtrar a los 250 tokens más frecuentes.
6. Calcular tf-idf.
...
text_recipe <- recipe(sender ~ ., data = chat_train) %>% 
  step_mutate(sender = as.character(sender),
              sender = if_else(sender %in% sender_target,
                               sender,
                               "Otro"),
...
A continuación, se crean las especificaciones de modelos multiclase: un clasificador bayesiano y un modelmultinomial con regularización L2.
...
Ahora, se ajustan los modelos en los conjuntos de validación cruzada.
