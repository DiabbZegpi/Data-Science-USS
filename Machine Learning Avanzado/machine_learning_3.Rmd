...
nnet_results_1 %>% 
  ...
  annotate("text", x = 2.6, y = .642, size = 5,
           label= "Red Neuronal\nAccuracy estimado ± error estándar") +
...
El entrenamiento de la red neuronal profunda será llevado a cago utilizando el paquete `keras`. Este framework necesita que los datos de entrenamiento y prueba ingresen al modelo en forma matricial.
...
Ahora se reentrena el modelo con la mejor combinación alcanzado.
...
```{r echo=FALSE, eval=FALSE}
save_model_tf(model_keras, file = here("Machine Learning Avanzado"))
```

```{r echo=FALSE}
model_keras <- load_model_tf("Machine Learning Avanzado")
```


Para finalizar, se hacen predicciones e el conjunto de entrenamiento.
...
La red neuronal profunda supera al model SVM en accuracy. Además, debido al efecto de SMOTE, la amtriz de confusión del SVM tiene más observaciones. Esto debió ser corregido durante la etapa de creación de la matriz `x_train`, pero no altera demasiado los resultados.
