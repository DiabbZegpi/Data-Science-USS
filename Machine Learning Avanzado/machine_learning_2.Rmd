...
titanic_train %>% 
  mutate(Title = create_title(Name)) %>% 
  group_by(Title) %>% 
...
Se observa que pese al traslape de las distribuciones, la edad sí puede ser caracterizada por el título de la pasajera, Finalmente, los valores faltantes de edad serán imputados con el algortimo *KNN* y utilizando la *distancia de Gower*, cuyos predictores serán las variables `Title`, `Sex`, `SibSp` y `Parch`.
...
### Resampling y Ajuste de hiperparámetros
...
Paralelamente, se declara la especificación del modelo de ranfom forests utilizando engine del paquete `ranger`, en *modo clasificación*. Los hiperparámetros a afinar se deifnen a continuación:

- **mtry**: un entero para el número de predictores que son muestreados aleatoriamente en cada separación al crear lo modelos de árbol.
...
Se elige usar un método de búsqueda en grilla para optimizar la combinación de hiperparámetros. El diseño de grilla escogido es de máxima entropía. Debido a que el parámetro mtry depende de las dimensiones del dataset, el framework de tidymodels requiere que se finalice el parámetro con el rango de dimensiones correctas.
...
rf_grid <- 
  grid_max_entropy(
    rf_spec %>% 
      parameters() %>% 
      update(mtry = range_set(mtry(), c(1, 11))),
    size = 20
  )
```

Una vez se tienen los componentes más importantes del framework de tidymodels (recipe, model specification, data resampling & tunning grid), se procede a crear un workflow que los haga interactuar.
