...
Afinar 100 combinaciones de hiperparámetros en 10 subconjuntos de datos implica ajustar 1000 veces el modelo de random forest, con una cantidad de árboles que varía entre las decenas y un par de miles. Para acelerar los cálculos, se permite que tidymodels acceda a los múltiples cores que tiene el computador mediante computación paralela, con `doParallel`.
...
La mejor combinación de hiperparámetros resultó ser trees: 281, mtry: 5 y min_n: 5. Esta combinación alcanzó un accuracy estimado de 0.843, con un error estándar estimado de 0.0142. Paralelamente, el área media bajo la curva ROC estimada es 0.881, con error estándar de 0.0157.
...
collect_predictions(rf_res) %>% 
  ...
  annotate(geom = "text", x = .45, y = .35, size = 4.5, family = "Noto Sans", fontface = "italic",
           label = "ROC AUC media = 0.881\nStd. Error = 0.0142", hjust = 0) +
  coord_fixed() +
...
```{r}
bind_rows(train = train_processed, validation = val_processed, test = test_processed, .id = "set") %>% 
  nest(data = all_of(2:ncol(.))) %>% 
  mutate(pred = map(data, ~predict(rf_fit, new_data = .x))) %>% 
  filter(set %in% c("train", "validation")) %>% 
