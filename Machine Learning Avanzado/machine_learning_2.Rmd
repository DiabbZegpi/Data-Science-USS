...
La mejor combinación de hiperparámetros resultó ser trees: 1808, mtry: 5 y min_n: 10. Esta combinación alcanzó un accuracy estimado de 0.843, con un error estándar estimado de 0.0142. Paralelamente, el área media bajo la curva ROC estimada es 0.881, con error estándar de 0.0157.
...
bind_rows(train = train_processed, validation = val_processed, .id = "set") %>% 
  ...
  geom_line(size = 1) +
  geom_label(aes(label = round(value, 2)), size = 5, fontface = "bold", label.size = 1) +
  scale_color_manual(values = c("#a92759", "#354fa9", "#018666")) +
  coord_cartesian(ylim = c(.8, 1)) +
  facet_wrap(~metric) +
  theme(legend.position = "none") +
  labs(x = "Set", y = "Valor de la métrica", 
       title = "¿Qué métrica es mejor para seleccionar hiperparámetros?",
